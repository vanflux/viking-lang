...
export class Instruction {
  ...
  static disassemble(code: number, architecture: Architecture) {
    ...
    if (imm === 1) {
      ...
      let operands = [new Operand(rst, Operand.REGISTER), new Operand(immediate, Operand.LITERAL)];
      ...
      let operands = [new Operand(rst, Operand.REGISTER), new Operand(rsa, Operand.REGISTER), new Operand(rsb, Operand.REGISTER)];
  ...
  getType() {
    if (this.operands.length === 2 && this.operands[0].getType() === Operand.REGISTER && this.operands[1].getType() === Operand.LITERAL)
      return Instruction.TYPE_I;
    if (
      this.operands.length === 3 &&
      this.operands[0].getType() === Operand.REGISTER &&
      this.operands[1].getType() === Operand.REGISTER &&
      this.operands[2].getType() === Operand.REGISTER
    )
      return Instruction.TYPE_R;
    return null;
  }
